...
Func Login_LoadMS($_userNameToGet = "")
   ;Read from the specific File for this login
   _SqlConnect()

   If Not GetOldestActiveLoginMS() then
	  LogMessage("Getting Login Failed - " & _SQL_GetErrMsg())
	  _SQL_Close()
	  return False
   EndIf
...
Func Login_PINMS()
   ...
   Local $PINFileName = "MobileStrike/MSFilePIN.au3"
...
Func Login_PINGoW()
   ...
   Local $PINFileName = "GameOfWar/GoWFilePIN.au3"
   If FileExists($PINFileName) = 1 Then
	  FileDelete($PINFileName)
   EndIf
